...
from servicenow_mcp.tools.automation_tools import (
    ...
    run_scheduled_job_now as run_scheduled_job_now_tool,
)
from servicenow_mcp.tools.customization_tools import (
    ...
    enable_client_script as enable_client_script_tool,
    disable_client_script as disable_client_script_tool,
    delete_client_script as delete_client_script_tool,
    enable_ui_action as enable_ui_action_tool,
    disable_ui_action as disable_ui_action_tool,
...
def get_tool_definitions(
    ...
    tool_definitions: Dict[str, ToolDefinition] = {
        ...
        "enable_client_script": (
            enable_client_script_tool,
            EnableClientScriptParams,
            dict,
            "Enable a client script (sys_script_client) by setting active=true.",
            "json",
        ),
        "disable_client_script": (
            disable_client_script_tool,
            DisableClientScriptParams,
            dict,
            "Disable a client script (sys_script_client) by setting active=false.",
            "json",
        ),
        "delete_client_script": (
            ...
            DeleteClientScriptParams,
            dict,
            "Permanently delete a client script record from sys_script_client.",
            "json",
        ),
        "enable_ui_action": (
            enable_ui_action_tool,
            EnableUiActionParams,
            dict,
            "Enable a UI action button/link (sys_ui_action) by setting active=true.",
            "json",
        ),
        "disable_ui_action": (
            disable_ui_action_tool,
            DisableUiActionParams,
            dict,
            "Disable a UI action button/link (sys_ui_action) by setting active=false.",
            "json",
        ),
        "get_ui_policy": (
            get_ui_policy_tool,
            GetUiPolicyParams,
            dict,
            "Get a single UI policy (sys_ui_policy) by sys_id.",
            "json",
        ),
        "create_ui_policy": (
            ...
            CreateUiPolicyParams,
            dict,
            "Create a new UI policy (sys_ui_policy) for a table with optional condition and field rules.",
            "json",
        ),
        "update_ui_policy": (
            ...
            UpdateUiPolicyParams,
            dict,
            "Update an existing UI policy (sys_ui_policy). Sparse update — only provided fields change.",
            "json",
        ),
        "create_ui_policy_action": (
            ...
            CreateUiPolicyActionParams,
            dict,
            "Create a field-level action rule on a UI policy (sys_ui_policy_action). "
            "mandatory/visible/disabled accept True/False/None — None maps to ServiceNow 'leave'.",
            "json",
        ),
        "list_ui_policy_actions": (
            ...
            ListUiPolicyActionsParams,
            dict,
            "List all field-level action rules for a UI policy (sys_ui_policy_action).",
            "json",
        ),
        ...
        "update_scheduled_job": (
            ...
            UpdateScheduledJobParams,
            dict,
            "Update a scheduled job (sys_trigger) — name, script, or schedule. Only non-None fields are patched.",
            "json",
        ),
        "run_scheduled_job_now": (
            run_scheduled_job_now_tool,
            RunScheduledJobNowParams,
            dict,
            "Trigger a scheduled job to run immediately (sets trigger=true on sys_trigger). "
            "Executes on next scheduler poll (~1 minute).",
            "json",
        ),
